...
export function MeScreen() {
  ...
  const items =
    view === "missions"
      ? missions
      : missions.filter((m) => registeredMissions.has(m.id));
          ...
          <div className="flex gap-[18px] mt-2">
            <div>
              <div className="font-bold text-[28px] leading-none">
                {TRIZAC_DATA.user.heuresBenevolat} h
              </div>
            ...
            <div>
              <div className="font-bold text-[28px] leading-none">
                {TRIZAC_DATA.user.missionsTerminees + registeredMissions.size}
              </div>
      ...
      <div className="px-[18px] pb-2 flex gap-1.5">
        {(
          ...
          return (
            ...
            >
              {tab.label}
              {tab.id === "mes" && registeredMissions.size > 0 && (
                <span className="ml-1.5 opacity-80">· {registeredMissions.size}</span>
              )}
            </button>
          );
        })}
            ...
            return (
              <Surface key={m.id}>
                ...
                <div className="flex items-center justify-between mt-3 pt-2.5 border-t border-line-soft">
                  ...
                  <Button
                    size="sm"
                    variant={registered ? "secondary" : complet && !registered ? "ghost" : "primary"}
                    aria-pressed={registered}
                    onClick={() => {
                      if (complet && !registered) {
                        show({ tone: "info", title: "Mission complète", desc: m.titre });
                        return;
                      }
                  ...
                  >
                    {registered ? "Inscrit·e ✓" : complet ? "Complet" : "Je m'inscris"}
                  </Button>
                </div>
              </Surface>
            );
