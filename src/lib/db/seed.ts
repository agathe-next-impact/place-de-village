...
db.insert(schema.signalementHistory)
  .values([
    { signalementId: "s1", etat: "signale", agentId: null },
...
// Signaux pour donner du contexte
const sigData = [
  ["idx1", "vis", 8],
  ["idx1", "important", 14],
  ["idx1", "contribuer", 5],
...
for (const [sugId, type, count] of sigData) {
  for (let i = 0; i < count; i++) {
    const userId = `seed-${sugId}-${type}-${i}`;
    db.insert(schema.users).values({ id: userId, email: `${userId}@seed.local`, name: `Habitant ${i}` }).onConflictDoNothing().run();
    db.insert(schema.signalEmissions).values({ userId, suggestionId: sugId, type }).run();
  }
}

db.insert(schema.discussions)
  .values([
...
for (const [discId, [acc, nuance, obj, q]] of Object.entries(contribCounts)) {
  for (let i = 0; i < acc; i++) {
    db.insert(schema.contributions).values({ discussionId: discId, type: "accord", texte: `Contribution accord #${i + 1}`, auteurId: "u2", auteur: "Habitant" }).run();
  }
  for (let i = 0; i < nuance; i++) {
    db.insert(schema.contributions).values({ discussionId: discId, type: "nuance", texte: `Contribution nuance #${i + 1}`, auteurId: "u3", auteur: "Habitant" }).run();
  }
  for (let i = 0; i < obj; i++) {
    db.insert(schema.contributions).values({ discussionId: discId, type: "objection", texte: `Contribution objection #${i + 1}`, auteurId: "u4", auteur: "Habitant" }).run();
  }
  for (let i = 0; i < q; i++) {
    db.insert(schema.contributions).values({ discussionId: discId, type: "question", texte: `Contribution question #${i + 1}`, auteurId: "u5", auteur: "Habitant" }).run();
  }
}
...
// 87 soutiens fictifs sur p1, 142 sur p2
for (let i = 0; i < 87; i++) {
  const u = `support-p1-${i}`;
  db.insert(schema.users).values({ id: u, email: `${u}@seed.local`, name: `Soutien ${i}` }).onConflictDoNothing().run();
  db.insert(schema.supports).values({ userId: u, propositionId: "p1" }).run();
}
for (let i = 0; i < 142; i++) {
  const u = `support-p2-${i}`;
  db.insert(schema.users).values({ id: u, email: `${u}@seed.local`, name: `Soutien ${i}` }).onConflictDoNothing().run();
  db.insert(schema.supports).values({ userId: u, propositionId: "p2" }).run();
}
...
for (const [u, m] of insc) {
  db.insert(schema.missionRegistrations).values({ userId: u, missionId: m }).run();
}
...
db.insert(schema.petitesAnnonces)
  .values([
    { id: "pa1", type: "don", cat: "Mobilier", titre: "Canapé 3 places", description: "Bon état général, tissu beige. À récupérer sur place ce week-end.", auteurId: "u3", auteur: "Marie D.", expiresAt: new Date((now + month) * 1000) },
    { id: "pa2", type: "pret", cat: "Outils", titre: "Décolleuse à papier peint", description: "Disponible le week-end, contre une bouteille de vin.", auteurId: "u10", auteur: "Romain B.", expiresAt: new Date((now + month) * 1000) },
    { id: "pa3", type: "echange", cat: "Jardin", titre: "Pommes contre œufs", description: "Mes pommes contre vos œufs frais, à voir au cas par cas.", auteurId: "u6", auteur: "Hélène P.", expiresAt: new Date((now + month) * 1000) },
  ])
  .run();

// ── Pôle 6 : Réservations ────────────────────────────────────────────
db.insert(schema.equipements)
  .values([
